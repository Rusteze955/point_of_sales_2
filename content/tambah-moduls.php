...
if (isset($_GET['delete'])) {
    ...
    $queryModulsDetails = mysqli_query($config, "SELECT file FROM moduls_details WHERE id_modul = '$id_user'");
    $rowModulsDetails   = mysqli_fetch_assoc($queryModulsDetails);
    unlink("uploads/" . $rowModulsDetails['file']);

    $queryDelete = mysqli_query($config, "DELETE FROM moduls_details WHERE id = '$id_user'");
...
$queryModul = mysqli_query($config, "SELECT majors.name as majors_name, instructors.name as instructors_name, moduls.* 
FROM moduls 
                        ...
                        <tbody>
                            <?php foreach ($rowDetailModuls as $index => $rowDetailModul): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a target="_blank" href="?page=tambah-moduls&download=<?php echo urlencode($rowDetailModul['file']) ?>"><?php echo $rowDetailModul['file'] ?><i class="bi bi-download"></i>
                                        </a>
                                    </td>
                                </tr>
                            <?php endforeach ?>
...
<script>
    ...
    button.addEventListener("click", function() {
        ...
        tr.innerHTML = `
        <td><input type='file' name='file[]'></td><td><button class="btn btn-danger">Delete</button></td>`;
