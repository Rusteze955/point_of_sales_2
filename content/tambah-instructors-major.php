...
if (isset($_GET['delete'])) {
    ...
    $queryDelete = mysqli_query($config, "DELETE FROM instructor_major WHERE id = '$id_instructor'");
    if ($queryDelete) {
        header("location:?page=tambah-instructors-major&id=" . $id_instructor . "&hapus=berhasil");
    } else {
        header("location:?page=tambah-instructors-major&id=" . $id_instructor . "&hapus=gagal");
    }
}
...
if (isset($_POST['id_major'])) {
    ...
    if (isset($_GET['edit'])) {
        ...
        header("location:?page=tambah-instructors-major&id=" . $id_instructor . "&ubah=berhasil");
    } else {
        $insert = mysqli_query($config, "INSERT INTO instructor_major (id_instructor, id_major) VALUES ('$id_instructor', '$id_major')");
        header("location:?page=tambah-instructors-major&id=" . $id_instructor . "&tambah=berhasil");
    }
}
...
$queryInstructorMajor = mysqli_query($config, "SELECT majors.name, instructor_major.id, id_instructor FROM instructor_major LEFT JOIN majors ON instructor_major.id_major = majors.id WHERE id_instructor ='$id_instructor' ORDER BY instructor_major.id desc");
$rowInstructorMajors = mysqli_fetch_all($queryInstructorMajor, MYSQLI_ASSOC);


$queryEdit = mysqli_query($config, "SELECT * FROM instructor_major ORDER BY id DESC");
$rowEdit = mysqli_fetch_assoc($queryEdit);
                ...
                <?php if (isset($_GET['edit'])): ?>
                    <form action="" method="post">
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="" class="from-label">Major Name</label>
                        ...
                        <tbody>
                            ...
                            foreach ($rowInstructorMajors as $index => $rowInstructorMajor): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="?page=tambah-instructors-major&id=<?php echo $rowInstructorMajor['id_instructor'] ?>&edit=<?php echo $rowInstructorMajor['id'] ?>" class="btn btn-primary">Edit</a>
                                        <a onclick="return confirm('Are you sure wanna delete this data??')" href="?page=tambah-instructors-major&delete=<?php echo $rowInstructorMajor['id'] ?>&id_instructor=<?php echo $rowInstructorMajor['id_instructor'] ?>" class="btn btn-danger">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach ?>
                        </tbody>
